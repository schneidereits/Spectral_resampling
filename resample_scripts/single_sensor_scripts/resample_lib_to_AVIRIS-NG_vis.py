...
# ------------------------
RESAMPLED_RAW_PATH = "spectral_library_resampled_AVIRIS-NG.csv"
RESAMPLED_INTERP_PATH = "spectral_library_resampled_AVIRIS-NG_interpolated.csv"
ORIGINAL_LIBRARY_PATH = "99_library_joined_with_lake.csv"
...
# ------------------------
meta_cols = ["class", "category_1", "category_2", "id_lib", "source"]
...
plot_nrmse_vs_original(resampled_raw_df, spectral_lib, wavelength_cols, "id_lib", "Normalized RMSE Across Bands: RAW vs Original")
plot_nrmse_vs_original(resampled_interp_df, spectral_lib, wavelength_cols, "id_lib", "Normalized RMSE Across Bands: interpolated vs Original")
...
def plot_spectra_comparison(resampled_df, ref_df, original_df, common_wavelengths, classes):
    for cls in classes:
        cls_ids = resampled_df[resampled_df['class'] == cls]['id_lib']
        ...
        for sid in cls_matching_ids:
            spec1 = resampled_df.set_index('id_lib').loc[sid, common_wavelengths].astype(float).values
            plt.plot([float(w) for w in common_wavelengths], spec1, alpha=0.9, linestyle=':', color="blue", linewidth=2.5)

            spec2 = ref_df.set_index('id_lib').loc[sid, common_wavelengths].astype(float).values
            plt.plot([float(w) for w in common_wavelengths], spec2, alpha=0.5, linestyle='--', color='red', linewidth=2.5)

            spec3 = original_df.set_index('id_lib').loc[sid, [str(int(float(w))) for w in common_wavelengths]].astype(float).values
        ...
        plt.legend(["Resampled (no interpolation)", "Resampled (interpolated)", "Original ASD Library"])
        plt.xlabel("Wavelength (nm)")
